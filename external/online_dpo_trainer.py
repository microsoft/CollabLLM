...
class OnlineDPOTrainer(Trainer):
    ...
    def __init__(
        ...
        with PartialState().local_main_process_first():
            ...
            if eval_dataset is not None:
                ...
                # newly added
                eval_dataset = eval_dataset.select_columns(['prompt_input_ids', 'prompt_attention_mask'])
                eval_dataset = eval_dataset.rename_column('prompt_input_ids', 'input_ids')
                eval_dataset = eval_dataset.rename_column('prompt_attention_mask', 'attention_mask')
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Union[str, Dataset]] = None) -> DataLoader:
        ...
        # def collator(data):
        #     return {key: torch.concat([d[key].view(1, -1) for d in data]) for key in data[0]}
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], **judge_kwargs) -> torch.Tensor:
        model.train()
        ...
        with torch.no_grad():
            responses = self.tokenizer.batch_decode(completion_ids, skip_special_tokens=True)
            new_inputs = {'qa': inputs['qa'][0], 'chat': inputs['chat'][0]}
            del inputs
            scores = self.judge.judge(new_inputs, responses).to(prompt_completion_ids.device)

            # generate random numbers for debugging
            # scores = torch.randn(len(prompt_completion_ids), device=prompt_completion_ids.device)
